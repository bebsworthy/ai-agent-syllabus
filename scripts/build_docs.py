...
def source_to_dest(filename: str) -> str:
    ...
    if filename == "README.md":
        return "index.md"
    # M01-How-LLMs-Work-workshop.md -> m01-workshop.md
    m = re.match(r"(M\d\d)-.*-workshop\.md$", filename, re.IGNORECASE)
    if m:
        return f"{m.group(1).lower()}-workshop.md"
    # M01-How-LLMs-Work.md -> m01-how-llms-work.md
    return filename.lower()


def fix_tier_prefixes(content: str) -> str:
    """Replace source tier directory names with docs directory names."""
...
def fix_links(content: str) -> str:
    ...
    def replace_link(m):
        ...
        # These are flat in our docs structure: m11-post-deployment.md
...
def remove_manual_nav(content: str) -> str:
    """Remove *Next: ...* footer lines (navigation.footer replaces them)."""
...
def extract_title(content: str, fallback: str) -> str:
    """Return the first H1 heading, or fallback if not found."""
...
def main() -> None:
    ...
    print(f"\nDone. Run `mkdocs serve` or `mkdocs build --strict` next.")
